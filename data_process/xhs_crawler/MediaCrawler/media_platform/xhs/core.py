...
from var import crawler_type_var, request_keyword_var, request_city_var, request_subarea_var
...
class XiaoHongShuCrawler(AbstractCrawler):
    ...
    async def search(self) -> None:
        ...
        for keyword in self.config.KEYWORDS:
            ...
            request_city_var.set(self.config.CITY)
            request_subarea_var.set(self.config.SUBAREA)
    ...
    async def launch_browser(
        ...
        if self.config.SAVE_LOGIN_STATE:
            ...
            user_data_dir = os.path.join(os.getcwd(), "browser_data",
                                         self.config.USER_DATA_DIR % self.platform)  # type: ignore
