...
from var import request_keyword_var, request_city_var, request_subarea_var
...
async def update_xhs_note(note_item: Dict):
    ...
    if config.IS_SAVED_DATABASED:
        ...
        source_keywords = request_keyword_var.get().replace('notes_', "")
        source_city = request_city_var.get()
        source_subarea = request_subarea_var.get()
        if len(source_subarea) > 0:
            source_city = source_city + '/' + source_subarea
        pathlib.Path(f"../../../data/xhs_crawl_raw/{source_city}/notes/").mkdir(parents=True, exist_ok=True)
        with open(f"../../../data/xhs_crawl_raw/{source_city}/notes/{source_keywords}.csv", mode='a+', encoding="utf-8-sig", newline="") as f:
            writer = csv.writer(f)
            if f.tell() == 0:
...
async def update_xhs_note_comment(note_id: str, comment_item: Dict):
    ...
    if config.IS_SAVED_DATABASED:
        ...
        if len(source_subarea) > 0:
            source_city = source_city + '/' + source_subarea
        pathlib.Path(f"../../../data/xhs_crawl_raw/{source_city}/comment/").mkdir(parents=True, exist_ok=True)
        with open(f"../../../data/xhs_crawl_raw/{source_city}/comment/{source_keywords}.csv", mode='a+', encoding="utf-8-sig", newline="") as f:
            writer = csv.writer(f)
            if f.tell() == 0:
                writer.writerow(local_db_item.keys())
            writer.writerow(local_db_item.values())
