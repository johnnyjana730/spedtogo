...
class ScrapeGoogleSearchGP(BaseTask):
    ...
    def run(self, driver, data):
        ...
        if os.path.exists("output/search_gp/" + self.city + '/') == False:
            os.makedirs("output/search_gp/" + self.city + '/')

        Output.write_json(new_results, "search_gp/" + self.city + '/' + pydash.kebab_case(keyword))
        # Output.write_csv(new_results, pydash.kebab_case(keyword))

        filepath = os.path.join("output/search_gp/", self.city + '/' + keyword_kebab + ".json")
        with open(filepath, 'r') as f:
                file_content = f.read()
                file_hash = hashlib.md5(file_content.encode()).hexdigest()                      
