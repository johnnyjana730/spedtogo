...
# # from google_search_utils import scrape_entity_info
# import random #for random sleep to avoid DOS

# def find_entity_link_candidate(google_info_dic):
#     query_keyword_set = {}
#     for place_name in google_info_dic:
#         place_data = google_info_dic[place_name]
#         notes_to_keywords = place_data['notes_to_keywords']

#         for keyword, word_match_valid in notes_to_keywords:
#             if word_match_valid is False:
#                 query_keyword_set[keyword] = {"found_place_name": place_name}

#     return query_keyword_set


# def google_search_entity_link(name, place_name, city_name, entity_link):

#     print('name = ', name, 'place_name = ', place_name, 'city_name = ', city_name)
    
#     if place_name in entity_link:
#         return entity_link[place_name]["match_flag"]

#     query = place_name + " " + city_name
...
#         try:
#             # tmp = scrape_entity_info(place_name+'+'+city_name)
#             # if fuzz.partial_ratio(tmp, name) > 90:
#             #     flag = True  

#             # for j in search(query, sleep_interval=5, num_results=10, advanced=True):
#             for j in search(query, advanced=True, num_results=10, sleep_interval=random.randint(10,20)):
#                 title = j.title
#                 description = j.description
#                 tmp.append([title,description])

#                 similarity_ratio_p1 = fuzz.partial_ratio(title, name)
#                 similarity_ratio_p2 = fuzz.partial_ratio(description, name)
#                 if similarity_ratio_p1 > 90 or similarity_ratio_p2 > 90:
#                     flag = True
#             finished = True
...
#     time.sleep(10)
#     entity_link[place_name] = {"google_result": tmp, "found_place_name": name, "match_flag": flag}

#     return flag


# def google_search_entity_link_bot(name, place_name, city_name, entity_link):
...
#         try:
#             # for j in search(query, sleep_interval=5, num_results=10, advanced=True):
#             for j in search(query, advanced=True, num_results=10, sleep_interval=random.randint(50,60)):
#                 title = j.title
#                 description = j.description
#                 tmp.append([title,description])
